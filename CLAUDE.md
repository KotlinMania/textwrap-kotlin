# Claude Code Project Instructions — textwrap-kotlin

## Project Overview

This is **textwrap-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`textwrap`](https://crates.io/crates/textwrap).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/textwrap/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/textwrap>
