...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`textwrap`](https://crates.io/crates/textwrap).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `textwrap` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Word-wrap, fill, and dedent text
...
Original work copyrighted by the upstream `textwrap` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
